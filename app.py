...
# Sample training data (aapke real data se yeh replace kar sakte hain)
data = {
    'Temperature': [25, 28, 24, 26, 27, 29],  # Sample temperature values
    'Humidity': [80, 70, 85, 75, 65, 60],     # Sample humidity values
    'Pressure': [1015, 1010, 1013, 1012, 1018, 1019],  # Sample pressure values
    'Rain': [1, 0, 1, 0, 1, 0]               # 1 = Rain, 0 = No Rain (target variable)
}
...
def predict_rain(temperature, humidity, pressure):
    ...
    if prediction == 1:
        return "🌧️ **Rain is likely!** 🌧️"  # If rain is predicted
    else:
        return "☀️ **Rain is not likely.** ☀️"  # If no rain is predicted

# Streamlit UI to take user input and predict
# Streamlit title (this is displayed as the heading on the app)
st.title('🌦️ **Weather Prediction Model** 🌦️')
